...
Sessions 1-6, 9a-9c and 10 shipped. Python 3.11 package at `src/ehr_simulator/` with:
...
- **Tests**: 560 tests across 32 files. Two marker suites are deselected by default: 8 Playwright tests in `tests/e2e/` (`-m e2e`) and 2 `@pytest.mark.real_data` adapter smokes that need the local Geneva + MIMIC CSVs.
...
- `uv run pytest` — run the default suite (560 tests, parallelized via `pytest-xdist`; e2e + real-data deselected).
