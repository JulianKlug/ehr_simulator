...
- **Behavioral timing + divergence** (S10): `timepoint.enter`/`timepoint.exit` events (`web/timing_events.py`) — enter on the editable-frontier GET and on HTMX advance-into-next, exit (`reason` `advance|finish`) from `gating.advance()`. `timing.py` is the pure derivation (sort by `(server_ts, event_id)`; first ENTER = start; first EXIT after = end; no-enter → blank; exit-before-enter ignored; end<start → `TimingError`) plus the three wall-clock columns `timepoint_started_at`/`timepoint_ended_at`/`elapsed_seconds` appended to the export row. `divergence.py` (no `web/` imports) renders the rough per-patient divergence figure in plotnine-SVG: per-question panels (option proportions, numeric median+observations, free-text non-empty counts only), a newly-visible-data annotation panel (counts by canonical category via `VITAL_VAR_SET`/`LAB_VAR_SET` from `ingestion/canonical.py`, never raw values), a wall-clock elapsed panel, and a single-arm note. Arm → color and option → line-style are per-geom constants (deliberately **not** discrete scales, so >4 styles / >10 arms don't hit mizani palette limits and NaN). `divergence-view STUDY_CONFIG QUESTIONS --patient ID [--db-path P --out FIGURE.svg]` is the CLI. `main()` runs with `standalone_mode=True` and maps refusals to `SystemExit(1)`; a **subprocess** test pins OS status 1/0 (CliRunner can't).
- **Logging** (`logging.py`): structlog JSONL pipeline rolling at UTC midnight to `./logs/current.jsonl`.
- **Tests**: 550 tests across 32 files. Two marker suites are deselected by default: 8 Playwright tests in `tests/e2e/` (`-m e2e`) and 2 `@pytest.mark.real_data` adapter smokes that need the local Geneva + MIMIC CSVs.
...
- `uv run pytest` — run the default suite (550 tests, parallelized via `pytest-xdist`; e2e + real-data deselected).
