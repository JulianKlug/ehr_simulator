...
- **Add a Geneva-side integration test asserting `_read_features_csv` defensive issue-emission catches unrecognized source values in the real Geneva fixture.** S4 added test_shared.py #7 covering the synthetic case; the Geneva fixture should also exercise the defensive path so a future upstream Geneva CSV that adds e.g. `notes` rows (mirroring MIMIC) surfaces as `IngestionIssue(reason="unrecognized source value: ...")`, not silent loss. Cost: ~1 test (~15 LOC). Revival criterion: when S5 wires up the structlog WARNING (per existing S3 TODO "Emit a structlog WARNING when `_decode_categorical` falls back to argmax in lenient mode"), the same WARNING infrastructure can fire on unrecognized sources too. Depends on: S5.
...
- **Emit a structlog WARNING event when `_decode_categorical` falls back to argmax in lenient mode.** Resolution 1.2 of /plan-eng-review on S3 added an argmax + `IngestionIssue` fallback for ambiguous categorical decodes (multiple ≥0.5 values in one one-hot group after inverse-normalize). `IngestionIssue` gives row-level audit but isn't surfaced in real-time when `load_geneva` runs in CLI/server context. A structlog WARNING gives the S5 CLI loud failure signal and lets the S8 UI badge affected admission rows. Cost: structlog import in `geneva.py` (currently no logging dependency). Revival criterion: S5 CLI wired up (`validate-adapter` would be the natural emitter). Depends on: S5.
