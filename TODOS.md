...
- **S9a: lock down `events.kind` taxonomy with a `Literal` type alias.** S6 ships free-text `events.kind` (TEXT NOT NULL) with examples in the spec but no compile-time enforcement. S9a will add multiple producers (`panel.swap`, `answer.upsert`, `page.render-error`, etc.). Without a `EventKind = Literal["clinician.login", "clinician.logout", "panel.swap", ...]` type alias enforced in `events.append`, taxonomy drift is silent and analyses that filter by kind silently miss new variants. Cost at S9a: ~5 LOC + 1 mypy/runtime check. Revival criterion: when S9a's first new event-kind producer lands. Depends on: S9a.
