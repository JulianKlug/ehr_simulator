...
- **README.md persistence-paragraph update post-S6.** Lines 134-139 currently claim "only JSONL logs hit disk" — wrong after S6 ships SQLite + backups. Owned by `/document-release` post-S6 ship per CLAUDE.md skill routing, but tracking here so it doesn't slip if document-release isn't run. Update text: "JSONL logs at `logs/current.jsonl` + SQLite at `data/ehr_simulator.db` + automatic backups at `data/backups/<utc_iso>.db` after every researcher session." Depends on: S6 shipped.
...
- **S9b prerequisite: `required: bool = True` on `_QuestionBase`.** S9b's gate makes every question mandatory, including `free_notes`. Adding the defaulted field needs no `schema_version` bump but shifts every `config_hash` (`compute_config_hash_from_models` hashes the dumped model), so it must land before the first pilot answer is recorded. Depends on: S9b commit 1.
- **S9b authoring rule: multi-select questions need an explicit opt-out option** (`None of these`) whenever "nothing applies" is a real answer. An empty submission deletes the row and gating reads a missing row as unanswered. Document in the questions.yaml example when S9b ships.
